    ...
    const CTable = $('#data-table').DataTable({
        ...
        "createdRow": function (row, data) {
            ...
            if (name === "default"){
                ...
                link = "<div class='ellipsis'><a class='link' href='/branches/update/" + id + "' data-btn='link' title='" + id + "'>" + name + "</a></div>";
                ...
                if (result.value) {
                    deleteRequest(baseUrl + "/delete/" + deleteId, response => {
                        if (response.status === 200) {
                            toastAlertSuccessBigNoBtn("branch deleted!").then(()=>reLoadTable())
                        } else {
